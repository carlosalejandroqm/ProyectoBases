<%@page import="co.edu.ufps.cancha.entities.Producto"%>
<%@page import="co.edu.ufps.cancha.dao.ProductoDAO"%>
	...
	<div class="container">
		...
		<table class="table table-bordered table-hover">
			...
			<tbody>
				<!--   for (Todo todo: todos) {  -->
				<%
					for (Producto p : new ProductoDAO().list()) {
				%>
				<tr>
					...
					<td><%=p.getCantidad()%></td>
					<td><%if(p.getConsumible()==null){%>
						<%="Consumible"%>
					<%}else{%>
						<%="Implemento"%>
					<%}%></td>
					<td><%=p.getBodega().getIdInventario()%></td>
					<td><a
						href="<%=request.getContextPath()%>/ProductoServlet?action=showedit&id=<%=p.getIdProducto()%>">Editar</a></td>&nbsp;&nbsp;&nbsp;&nbsp;
					<td>	<a href="ProductoServlet?action=delete&id=<%=p.getIdProducto()%>">Eliminar</a></td>
					<!-- } -->
				</tr>
				<%
					}
		...
		<div style="margin-bottom: 15px;">
			<a href="../indexAdministrador.jsp"><button type="submit"
					class="btn btn-success" id="listado">Volver</button></a>
					<a href="crearProducto.jsp"><button type="submit"
					class="btn btn-success" id="listado">Registrar Producto</button></a>
		</div>
	</div>
	<!--Modal-->
